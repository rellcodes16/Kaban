...
function Button({ children, icon, onClick, type }) {
    ...
    return <li className="list-none">
        <button onClick={handleClick} className="w-full text-left bg-none border-none transition-all flex items-center gap-3">{icon}<span className={`${type ==='danger-menu' && 'text-red-700' }`}>{children}</span></button>
    </li>;
}
